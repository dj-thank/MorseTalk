...
model="${MORSETALK_AI_MODEL:-gemma4:e2b-it-qat}"
docker run -d --name morsetalk-ollama --cpus=2 --memory=8g \
  -p 127.0.0.1:11434:11434 -e OLLAMA_NUM_PARALLEL=1 -e OLLAMA_NO_CLOUD=1 -e OLLAMA_CONTEXT_LENGTH=4096 ollama/ollama:0.34.0
ready=0
...
# The registry can time out independently of the 600-second outer budget.
...
for attempt in 1 2 3; do
  ...
  if timeout 600 docker exec morsetalk-ollama ollama pull "$model" > "$log" 2>&1; then
    pulled=1
