...
with sync_playwright() as p:
    ...
    page.set_viewport_size({'width':393,'height':852});page.wait_for_timeout(100);assert page.evaluate('document.documentElement.scrollWidth <= innerWidth');page.screenshot(path=str(OUT/'v02-mobile.png'),full_page=True);passed('393-pixel Android layout has no horizontal overflow')
    page.set_viewport_size({'width':320,'height':750});page.wait_for_timeout(100);assert page.evaluate('document.documentElement.scrollWidth <= innerWidth');passed('320-pixel compact layout has no horizontal overflow')
    ...
    page.set_viewport_size({'width':393,'height':852});page.wait_for_timeout(100);page.evaluate('scrollTo(0,0)');page.screenshot(path=str(OUT/'polish-mobile.png'),full_page=True)
    page.set_viewport_size({'width':1280,'height':1000});page.wait_for_timeout(100);page.screenshot(path=str(OUT/'polish-desktop.png'),full_page=True)
