...
export async function aiCapabilities(){
  if(hasNative())return nativeCall('aiCapabilities',{},5000);
...
export async function generateReply(messages,{model,endpoint,provider='ollama',backend='cpu',consent=false,signal}={}){
  ...
  if(hasNative()){
    ...
    try{result=await nativeCall('aiChat',{messages,model,endpoint,provider,backend,consent},95000);}catch(error){cancel();throw error;}finally{signal?.removeEventListener('abort',cancel);}
  }else{
