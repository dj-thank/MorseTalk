...
export class AcousticReceiver {
  constructor(options={}){
    this.samples=0;this.seen=new Map();
    const profiles=[
    ...
    this.decoders=profiles.map((profile,index)=>new PhoneticDecoder({...options,adaptive:true,threshold:profile.threshold,detection:profile,
      onMark:index===0?options.onMark:()=>{},onCharacter:index===0?options.onCharacter:()=>{},onLevel:index===0?options.onLevel:()=>{},onError:index===0?(message,observed)=>options.onError?.(message,{...observed,candidate:true,decoderProfile:index}):()=>{},
      onFrame:frame=>{
