...
export class AcousticReceiver {
  constructor(options={}){
    this.sampleRate=options.sampleRate??48000;this.samples=0;this.seen=new Map();this.progressProfiles=new Map();
    const profiles=[
    ...
    this.decoders=profiles.map((profile,index)=>new PhoneticDecoder({...options,adaptive:true,threshold:profile.threshold*(options.highFrequency?.05:1),detection:profile,
      ...
      onFrame:frame=>{
        ...
        if(last!==undefined&&this.samples-last<2*this.sampleRate)return;
