...
export function validateTiming({wpm = 40, frequency = 700, volume = 0.25} = {}) {
  if (!Number.isFinite(wpm) || wpm < 8 || wpm > 60) throw new Error('速度は8〜60 WPMにしてください。');
  if (!Number.isFinite(frequency) || frequency < 400 || frequency > 1200) throw new Error('周波数は400〜1200 Hzにしてください。');
...
export function codeToSegments(code, wpm = 40, { leadUnits = 7, tailUnits = 18 } = {}) {
  const {unit} = validateTiming({wpm});
...
export function synthesize(segments, { sampleRate = 16000, frequency = 700, volume = 0.25, maxSeconds = 600 } = {}) {
