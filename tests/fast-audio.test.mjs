...
test('Worklet failure rejects pending transmission and releases the microphone',async()=>{const e=install(),a=new FastAudio({wpm:1200}),events=[];try{e.autoEnd=false;await a.start(x=>events.push(x));const p=a.transmit(bytes);const rejected=assert.rejects(p,/停止/);a.node.onprocessorerror();assert.equal(a.closed,true);await rejected;assert.equal(events.length,1);assert.equal(events[0].kind,'fatal');assert.ok(e.events.includes('track-stop'));assert.equal(a.ctx,null);}finally{a.stop();e.restore();}});
test('Ended microphone track closes receiver without relying on the UI',async()=>{const e=install(),a=new FastAudio(),events=[];try{await a.start(x=>events.push(x));const end=e.track.onended;end();end();assert.equal(a.closed,true);assert.equal(events.length,1);assert.equal(events[0].kind,'fatal');assert.equal(a.ctx,null);}finally{a.stop();e.restore();}});
test('Late errors from a stopped worklet cannot affect a restarted receiver',async()=>{const e=install(),a=new FastAudio(),events=[];try{await a.start(x=>events.push(x));const late=a.node.onprocessorerror,ended=e.track.onended;a.stop();await a.start(x=>events.push(x));late();ended();assert.equal(events.length,0);assert.equal(a.closed,false);}finally{a.stop();e.restore();}});
test('Stop releases all resources even when a terminated worklet port throws',async()=>{const e=install(),a=new FastAudio();try{await a.start(()=>{});a.node.port.postMessage=()=>{throw new Error('Port already terminated');};assert.doesNotThrow(()=>a.stop());assert.equal(a.ctx,null);assert.equal(a.stream,null);assert.ok(e.events.includes('track-stop'));assert.ok(e.events.includes('context-close'));assert.doesNotThrow(()=>a.stop());}finally{try{a.stop();}catch{}e.restore();}});
